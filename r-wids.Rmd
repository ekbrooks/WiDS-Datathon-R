...
final_testing_imputed <- na.roughfix(final_testing)
final_testing_imputed %>%
  select(everything()) %>%
  summarise_all(funs(sum(is.na(.))))

submission_forest <- randomForest(treatment_pd ~., training, ntree = 500, mtry = 2)
final_pred <- predict(submission_forest, final_testing_imputed)
summary(final_pred)
